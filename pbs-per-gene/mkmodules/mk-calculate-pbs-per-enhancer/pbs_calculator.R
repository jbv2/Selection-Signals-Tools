...
file_dir <- args[1]
# png_file <- args[2]
# pbs_file <- gsub(pattern = ".png", replacement = ".tsv", x = png_file)
pbs_file <- args[2]
dictionary <- args[3]
pop1_pattern <- args[4]
pop2_pattern <- args[5]
...
csv.names <- grep(pattern = "A", x = csv.names,value = T, invert = T)
csv.names <- grep(pattern = "B", x = csv.names,value = T, invert = T)
