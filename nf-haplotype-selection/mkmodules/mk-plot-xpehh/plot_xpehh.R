# ---------------------------
##
## Script name: Plot XP-EHH
##
## Purpose of script: Plot XP-EHH results from rehh
##
...
# args[2] <- "test/data/xp_ehh_results.png"
...
my_y_title <- expression(paste("XP-EHH ", -log[10],italic("(p)")))
...
don <- results.df %>%
  ...
  mutate( is_annotate=ifelse(-log10(P)>3.5, "yes", "no"))

# Prepare X axis
axisdf <- don %>% group_by(CHR) %>% summarize(center=( max(BPcum) + min(BPcum) ) / 2 )

# Make the plot
xpehh.pl <- ggplot(don, aes(x=BPcum, y=-log10(P))) +
...
##Save xpehh plot
ggsave(filename = output, plot = xpehh.pl, device = "png", dpi = 300, width = 10, height = 6, units = "in")
